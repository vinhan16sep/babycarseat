                ...
                <div class="card">
                    <div class="card-body offset-1 col-lg-10">
                        <div class="basic-form">
                            <form role="form" method="POST" action="{{ route('update-knowledge', ['id' => $object->id, 'callback' => $callback]) }}" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                ...
                                <div class="form-group{{ $errors->has('knowledge_category_id') ? ' has-error' : '' }}">
                                    <label>Chọn quốc gia <span class="my-required">*</span></label>
                                    <select class="form-control w-30" name="knowledge_category_id" value="{{ old('knowledge_category_id', $object->knowledge_category_id) }}" id="selectCountry">
                                        <option></option>
                                        @foreach ($activedCategories as $item)
                                            <option value="{{$item->id}}" {{ old('knowledge_category_id', $object->knowledge_category_id) == $item->id ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('knowledge_category_id'))
                                        <span style="color:red;">{{ $errors->first('knowledge_category_id') }}</span>
                                    </span>
                                    @endif
                                </div>
                                ...
                                <a type="button" href="{{ route('list-knowledge') }}" class="btn btn-default btn-outline"><i class="ti-back-left icon-black"></i>&nbsp;&nbsp;Quay lại</a>
