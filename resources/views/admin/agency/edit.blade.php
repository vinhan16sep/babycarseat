                ...
                <div class="card">
                    <div class="card-body offset-1 col-lg-10">
                        <div class="basic-form">
                            <form role="form" method="POST" action="{{ route('update-agency', ['id' => $object->id, 'callback' => $callback]) }}" enctype="multipart/form-data">
                                @csrf
                                @method('put')

                                <div class="form-group{{ $errors->has('city_id') ? ' has-error' : '' }}">
                                    <label>Chọn tỉnh/thành phố <span class="my-required">*</span></label>
                                    <select class="form-control w-30" name="city_id" value="{{ old('city_id', $object->city_id) }}">
                                        <option></option>
                                        @foreach ($cities as $item)
                                            <option value="{{$item->id}}" {{ old('city_id', $object->city_id) == $item->id ? 'selected' : '' }}>{{$item->name}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('address') ? ' has-error' : '' }}">
                                    <label>Địa chỉ </label>
                                    <input type="text" name="address" value="{{ old('address', $object->address) }}" class="form-control" maxlength="255">
                                ...
                                <div class="form-group{{ $errors->has('phone') ? ' has-error' : '' }}">
                                    ...
                                    <input type="text" name="phone" value="{{ old('phone', $object->phone) }}" class="form-control" maxlength="255">
                                    @if ($errors->has('phone'))
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea rows="5" name="map" class="form-control my-textarea">{{ old('map', $object->map) }}</textarea>
                                    @if ($errors->has('map'))
