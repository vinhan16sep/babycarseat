...
@php
    $meta = isset($country) ? "Rượu " . $country->name : "Rượu vang";
@endphp
    ...
    <div class="container custom_container">
        <div class="inner_menu float_left">
            <ul>
                ...
                <li class="active"><a href="#"> <span><i class="fas fa-angle-right"></i></span> {{ $country ? $country->name : "Rượu vang" }} </a></li>
            </ul>
        </div>
    </div>
</div>

<div class="bz_product_grid_content_main_wrapper float_left pd-0">
    <div class="container custom_container">
        <div class="row">
            <div class="col-md-12 box-country">
                @foreach($countries as $k => $val)
                    <a class="btn {{ ($country && $country->slug == $val->slug) ? "active" : "" }}" href="{{ route("country", ["country" => $val->slug]) }}">
                        {{ $val->name }}
                    </a>
                @endforeach
            </div>
                            ...
                            <div class="container">
                                <div id="sidebar-filter" class="d-block d-sm-block d-md-block d-lg-none d-xl-none">
                                    ...
                                    <div id='cssmenu-filter'>
                                        @include('components.sidebar', ["sidebar_type" => "mobile"])
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="pc-sidebar">
                            @include('components.sidebar', ["sidebar_type" => "pc"])
                        </div>
                    </div>
                </div>
            </div>
