...
class GrapeController extends AdminController
{
    /**
    ...
    public function index() {
        $list = Grape::orderBy('updated_at', 'desc')->paginate(10);
        return view('admin/grape/index', compact('list'));
    }

    public function create() {
        return view('admin/grape/create');
    }
    ...
    public function store(Request $request) {
        ...
        $model->updated_by = 1;
        if ($model->save()) {
            return redirect()->route('list-grape')->with('success', Config::get('constants.MESSAGE.CREATE_SUCCEEDED'));
        }
        return redirect()->route('create-grape')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
    }

    public function edit($id) {

        $object = Grape::find($id);

        // If object not found
        if ($object == null || $object->count() == 0) {
            return redirect()->route('list-grape')->with('error', Config::get('constants.MESSAGE.DATA_NOT_FOUND'));
        }

        return view('admin/grape/edit', [
            'object' => $object,
        ]);
    }
    ...
    public function update(Request $request, $id) {
        ...
        if ($object->save()) {
            return redirect()->route('edit-grape', ['id' => $id])->with('success', Config::get('constants.MESSAGE.UPDATE_SUCCEEDED'));
        }
        return redirect()->route('edit-grape', ['id' => $id])->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
    }
    ...
    private function validateStore($request) {
        $this->validate($request, [
            ...
            'slug' => 'required|max:255|unique:grapes',
        ], [
    ...
    private function validateUpdate($request, $id) {
        $this->validate($request, [
            'name' => 'required|max:255',
            'slug' => 'required|max:255|unique:grapes,slug,' . $id . ',id',
        ], [
            'name.required' => 'Chưa nhập tên',
            'slug.required' => 'Chưa có slug',
            'slug.unique' => 'Slug đã tồn tại',
    ...
    private function isUsing($id) {
        $productCnt = Product::where(['grape_id' => $id])->count();
