...
class KnowledgeController extends AdminController
{
    ...
    public function index() {
        $list = Knowledge::with(['category'])->orderBy('updated_at', 'desc')->paginate(10);
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $model->knowledge_category_id = $request->input('knowledge_category_id');
            ...
            $model->updated_by = 1;
            if ($model->save()) {
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.KNOWLEDGE_IMAGE'), $model->id);
                $upload = $this->uploadImage($path, $request);
                $model->image = $upload;
        
                if ($model->save()) {
                    DB::commit();
                    return redirect()->route('list-knowledge')->with('success', Config::get('constants.MESSAGE.CREATE_SUCCEEDED'));
                }
                DB::rollBack();
                return redirect()->route('create-knowledge')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
                
            }
            return redirect()->route('create-knowledge')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
        } catch (Exception $e) {
            DB::rollBack();
            return redirect()->route('create-knowledge')->with('error', $e->getMessage()); 
        }
    }

    public function edit($id) {

        $object = Knowledge::find($id);

        // If object not found
        if ($object == null || $object->count() == 0) {
            return redirect()->route('list-knowledge')->with('error', Config::get('constants.MESSAGE.DATA_NOT_FOUND'));
        }
    ...
    public function update(Request $request, $id) {
        ...
        try {
            ...
            $object->knowledge_category_id = $request->input('knowledge_category_id');
            ...
            if($request->hasfile('image')) {
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.KNOWLEDGE_IMAGE'), $id);
            ...
            if ($object->save()) {
                ...
                return redirect()->route('list-knowledge', $params)->with('success', Config::get('constants.MESSAGE.UPDATE_SUCCEEDED'));
            }
            DB::rollBack();
            return redirect()->route('edit-knowledge', [
                'id' => $id, 
    ...
    public function delete(Request $request) {
        ...
        try {
            ...
            $object = Knowledge::find($request['id']);
            // If object not found
            if ($object == null || $object->count() == 0) {
                return response()->json(['status' => 'error','msg' => Config::get('constants.MESSAGE.DATA_NOT_FOUND')], 404);
            }
            ...
            if ($object->delete()) {
                ...
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.KNOWLEDGE_IMAGE'), $request['id']);
    ...
    private function validateStore($request) {
        $this->validate($request, [
            ...
            'slug' => 'required|max:255|unique:knowledges',
            'image' => 'required|image|mimes:jpg,jpeg,png|max:2048',
        ], [
            'knowledge_category_id.required' => 'Chưa chọn danh mục',
    ...
    private function validateUpdate($request, $id) {
        $this->validate($request, [
            'knowledge_category_id' => 'required',
            'title' => 'required|max:255',
            'slug' => 'required|max:255|unique:knowledges,slug,' . $id . ',id',
    ...
    protected function getActivedCategories() {
        return KnowledgeCategory::where(['is_active' => '1'])->get();
    }
}
