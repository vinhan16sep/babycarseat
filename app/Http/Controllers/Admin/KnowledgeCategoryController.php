...
class KnowledgeCategoryController extends AdminController
{
    ...
    public function index() {
        $list = KnowledgeCategory::orderBy('updated_at', 'desc')->paginate(10);
        return view('admin/knowledge-category/index', compact('list'));
    }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            if ($model->save()) {
                ...
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.KNOWLEDGE_CATEGORY_IMAGE'), $model->id);
                $upload = $this->uploadImage($path, $request);
                $model->image = $upload;
            
                if ($model->save()) {
                    DB::commit();
                    return redirect()->route('list-knowledge-category')->with('success', Config::get('constants.MESSAGE.CREATE_SUCCEEDED'));
                }
                DB::rollBack();
                return redirect()->route('create-knowledge-category')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));

            }
            return redirect()->route('create-knowledge-category')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
        } catch (Exception $e) {
            DB::rollBack();
            return redirect()->route('create-knowledge-category')->with('error', $e->getMessage()); 
        }
    }

    public function edit($id) {

        $object = KnowledgeCategory::find($id);

        // If object not found
        if ($object == null || $object->count() == 0) {
            return redirect()->route('list-knowledge-category')->with('error', Config::get('constants.MESSAGE.DATA_NOT_FOUND'));
        }

        return view('admin/knowledge-category/edit', [
            'object' => $object,
        ]);
    }
    ...
    public function update(Request $request, $id) {
        ...
        try {
            ...
            if ($object->save()) {
                ...
                return redirect()->route('edit-knowledge-category', ['id' => $id])->with('success', Config::get('constants.MESSAGE.UPDATE_SUCCEEDED'));
            }
            DB::rollBack();
            return redirect()->route('edit-knowledge-category', [
                'id' => $id, 
    ...
    public function delete(Request $request) {
        ...
        try {
            ...
            $object = KnowledgeCategory::find($request['id']);
            // If object not found
            if ($object == null || $object->count() == 0) {
                return response()->json(['status' => 'error','msg' => Config::get('constants.MESSAGE.DATA_NOT_FOUND')], 404);
            }
            ...
            if ($object->delete()) {
                ...
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.KNOWLEDGE_CATEGORY_IMAGE'), $request['id']);
    ...
    private function isUsing($id) {
        $knowledgeCnt = Knowledge::where(['knowledge_category_id' => $id])->count();
