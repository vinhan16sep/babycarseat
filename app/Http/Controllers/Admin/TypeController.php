...
class TypeController extends AdminController
{
    ...
    public function index() {
        $list = Type::orderBy('updated_at', 'desc')->paginate(10);
        return view('admin/type/index', compact('list'));
    }
    ...
    public function store(Request $request) {
        ...
        $model->updated_by = 1;
        if ($model->save()) {
            return redirect()->route('list-type')->with('success', Config::get('constants.MESSAGE.CREATE_SUCCEEDED'));
        }
        return redirect()->route('create-type')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
    }
    ...
    public function edit($id) {
        ...
        if ($object == null || $object->count() == 0) {
            return redirect()->route('list-type')->with('error', Config::get('constants.MESSAGE.DATA_NOT_FOUND'));
        }
    ...
    public function update(Request $request, $id) {
        ...
        if ($object->save()) {
            return redirect()->route('edit-type', ['id' => $id])->with('success', Config::get('constants.MESSAGE.UPDATE_SUCCEEDED'));
        }
        return redirect()->route('edit-type', ['id' => $id])->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
    }
    ...
    private function validateStore($request) {
        $this->validate($request, [
            ...
            'slug' => 'required|max:255|unique:types',
        ], [
    ...
    private function validateUpdate($request, $id) {
        $this->validate($request, [
            'name' => 'required|max:255',
            'slug' => 'required|max:255|unique:types,slug,' . $id . ',id',
        ], [
            'name.required' => 'Chưa nhập tên',
            'slug.required' => 'Chưa có slug',
            'slug.unique' => 'Slug đã tồn tại',
    ...
    private function isUsing($id) {
        $productCnt = Product::where(['type_id' => $id])->count();
