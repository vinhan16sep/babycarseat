...
class GiftController extends AdminController
{
    ...
    public function index() {
        $list = Gift::orderBy('updated_at', 'desc')->paginate(10);
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $model->updated_by = 1;
            if ($model->save()) {
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.GIFT_IMAGE'), $model->id);
                $upload = $this->uploadImage($path, $request);
                $model->image = $upload;
        
                if ($model->save()) {
                    DB::commit();
                    return redirect()->route('list-gift')->with('success', Config::get('constants.MESSAGE.CREATE_SUCCEEDED'));
                }
                DB::rollBack();
                return redirect()->route('create-gift')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
                
            }
            return redirect()->route('create-gift')->with('error', Config::get('constants.MESSAGE.SOMETHING_ERROR'));
        } catch (Exception $e) {
            DB::rollBack();
            return redirect()->route('create-gift')->with('error', $e->getMessage()); 
        }
    }
    ...
    public function edit($id) {
        ...
        if ($object == null || $object->count() == 0) {
            return redirect()->route('list-gift')->with('error', Config::get('constants.MESSAGE.DATA_NOT_FOUND'));
        }
    ...
    public function update(Request $request, $id) {
        ...
        try {
            ...
            if($request->hasfile('image')) {
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.GIFT_IMAGE'), $id);
            ...
            if ($object->save()) {
                ...
                return redirect()->route('list-gift', $params)->with('success', Config::get('constants.MESSAGE.UPDATE_SUCCEEDED'));
            }
            DB::rollBack();
            return redirect()->route('edit-gift', [
                'id' => $id, 
    ...
    public function delete(Request $request) {
        ...
        try {
            ...
            if ($object->delete()) {
                ...
                $path = sprintf(Config::get('constants.FILE_STORAGE_PATH.GIFT_IMAGE'), $request['id']);
